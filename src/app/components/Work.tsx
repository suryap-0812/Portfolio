...
const PROJECTS = [
  {
    ...
    desc: 'Developed a real-time collaborative task management platform enabling users to form “Tribes” and manage shared tasks efficiently.',
    longDesc: 'Developed a real-time collaborative task management platform enabling users to form “Tribes” and manage shared tasks efficiently. Implemented a responsive ReactJS frontend with WebSocket-based live updates and a scalable NodeJS–PostgreSQL backend for secure group and task management.',
    ...
    codeUrl: '#',
    architecture: [
      { name: 'UI Client', desc: 'React / Tailwind CSS' },
      { name: 'Gateway', desc: 'Node.js WebSocket' },
      { name: 'Relational DB', desc: 'PostgreSQL Database' }
    ],
...
wss.on('connection', (ws, req) => {
  ...
  },
  {
    ...
    desc: 'Designed and developed a mobile application for CivicBridge that enables automated form parsing, translation, and voice assistant guidance.',
    longDesc: 'Designed and developed a mobile application for CivicBridge that enables automated form parsing, regional language translation, and interactive voice assistant guidance. The platform allows users and administrators to upload documents, extract complex fields, hear spoken instructions, and access simplified multilingual explanations through a premium interface.',
...
async def parse_civic_form(image_path: str, lang: str):
  ...
  },
  {
    ...
    desc: 'Developed a full-stack web application to track and compare product prices across multiple vendors with structured data scrapers.',
    longDesc: 'Developed a full-stack web application to track and compare product prices across multiple vendors. Implemented structured data modeling, RESTful services, and persistent storage to manage multiple price records per product, generate lowest-price reports, and analyze historical pricing trends efficiently.',
    image: 'https://images.unsplash.com/photo-1451187580459-43490279c0fa?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
  ...
  return (
    ...
    >
      <div>
        ...
        <div className="flex justify-between items-center mb-6 font-mono text-xs md:text-sm text-slate-500 uppercase tracking-widest">
        ...
        {/* Title */}
        <h3 className="font-display text-2xl text-white font-bold mb-3 tracking-tight group-hover:text-sky-400 transition-colors uppercase">
          {project.title}
        </h3>

        {/* Desc */}
        <p className="font-body text-slate-350 text-sm leading-relaxed mb-6">
          {project.desc}
        </p>
      </div>

      {/* Tags */}
      <div className="flex flex-wrap gap-2 pt-4 border-t border-white/5 font-mono text-xs md:text-sm text-slate-500 tracking-wider">
    ...
    <section id="work" className="relative min-h-screen py-24 px-6 md:px-12 theme-bg border-t theme-border">
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-16 gap-8 border-b border-white/5 pb-12">
          ...
          <p className="font-body text-slate-300 text-sm md:text-base leading-relaxed max-w-md text-left">
        ...
        <div className="mt-20 grid grid-cols-1 lg:grid-cols-3 border-t border-white/5">
          ...
          <div className="p-8 border-b lg:border-b-0 lg:border-r border-white/5 text-left hud-corner-cross">
            <span className="font-mono text-xs md:text-sm tracking-[0.25em] text-[#38bdf8] uppercase block mb-6">
              // OPERATIONAL_ACHIEVEMENTS
            </span>
            <ul className="space-y-6">
              <li>
                <h4 className="font-display text-sm md:text-base font-bold text-white mb-1 uppercase tracking-wide">
                  Eureka! 2025 Zonal Rounds
                </h4>
                <p className="font-body text-xs md:text-sm text-slate-400">
                  Shortlisted for regional selection phases, hosted by E-Cell, IIT Bombay (2025).
                </p>
              </li>
              <li>
                <h4 className="font-display text-sm md:text-base font-bold text-white mb-1 uppercase tracking-wide">
                ...
                <p className="font-body text-xs md:text-sm text-slate-400">
                  Selected in internal campus rounds for final submissions (2025).
                </p>
              </li>
            </ul>
          </div>
          ...
          {/* Column 2: Profiles */}
          <div className="p-8 border-b lg:border-b-0 lg:border-r border-white/5 text-left hud-corner-cross">
            <span className="font-mono text-xs md:text-sm tracking-[0.25em] text-[#3b82f6] uppercase block mb-6">
              // CODING_METRICS
            </span>
            <div className="grid grid-cols-2 gap-4">
              <a href="#" className="p-4 border border-white/5 hover:border-[#38bdf8]/35 bg-slate-950/40 rounded transition-colors text-left cursor-none" data-hover-label="LEETCODE">
                <span className="font-display text-sm font-bold text-white block mb-1">Leetcode</span>
                <span className="font-mono text-[0.68rem] md:text-xs text-slate-500">1551 Rating / Rank 275k / 133 Solved</span>
              </a>
              <a href="#" className="p-4 border border-white/5 hover:border-[#38bdf8]/35 bg-slate-950/40 rounded transition-colors text-left cursor-none" data-hover-label="SKILLRACK">
                <span className="font-display text-sm font-bold text-white block mb-1">Skillrack</span>
                <span className="font-mono text-[0.68rem] md:text-xs text-slate-500">1009 Problems Solved</span>
              </a>
            </div>
          </div>
          ...
          <div className="p-8 text-left hud-corner-cross">
            <span className="font-mono text-xs md:text-sm tracking-[0.25em] text-sky-400 uppercase block mb-6">
              // CREDENTIALS_REGISTER
            </span>
            <ul className="space-y-3 font-body text-xs md:text-sm text-slate-400">
              <li className="flex justify-between border-b border-white/5 pb-1">
                <span>Data Structures & Algorithms</span>
                <span className="font-mono text-[0.68rem] md:text-xs text-slate-500">Udemy (2025)</span>
              </li>
              <li className="flex justify-between border-b border-white/5 pb-1">
                <span>ReactJS Masterclass</span>
                <span className="font-mono text-[0.68rem] md:text-xs text-slate-500">Udemy (2024)</span>
              </li>
              <li className="flex justify-between border-b border-white/5 pb-1">
              ...
              <li className="flex justify-between">
                ...
                <span className="font-mono text-[0.68rem] md:text-xs text-slate-500">NVIDIA (2025)</span>
              </li>
            </ul>
          </div>
        </div>
          ...
          >
            ...
            >
              ...
              <div className="w-full lg:w-1/2 p-6 md:p-10 overflow-y-auto flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-white/5">
                <div>
                  <div className="flex items-center gap-3 mb-6 font-mono text-xs md:text-sm">
                  ...
                  <h3 className="font-display text-3xl md:text-4xl text-white font-bold mb-6 tracking-tight uppercase">
                    {activeProject.title}
                  </h3>

                  <div className="font-body text-slate-300 text-sm md:text-base leading-relaxed space-y-4 mb-8">
                    <p>{activeProject.desc}</p>
                    <p className="text-slate-450">{activeProject.longDesc}</p>
                  </div>

                  {/* Deconstruct and Expand pipeline map */}
                  <div className="mb-8">
                    <h4 className="font-mono text-xs md:text-sm tracking-wider uppercase text-slate-500 mb-4 flex items-center gap-2">
                      <Layers size={12} className="text-sky-400" /> SYSTEM ARCHITECTURE PIPELINE
                    </h4>
                    
                    <div className="border border-white/5 bg-slate-950 p-4 font-mono text-xs md:text-sm text-slate-450 space-y-3">
                ...
                <div>
                  <div className="flex flex-wrap gap-2 mb-6">
                    {activeProject.tags.map((tag) => (
                      <span key={tag} className="px-2 py-1 font-mono text-xs tracking-[0.1em] uppercase text-sky-400 bg-sky-950/20 border border-sky-900/30">
                        {tag}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex gap-4">
                    <a
                      ...
                      className="flex-1 py-3 px-5 text-center font-mono text-xs md:text-sm tracking-widest uppercase text-black bg-white hover:bg-slate-200 transition-colors cursor-none"
                    >
                    ...
                    <a
                      ...
                      className="py-3 px-5 font-mono text-xs md:text-sm tracking-widest uppercase text-white border border-white/10 hover:border-white/20 flex items-center justify-center gap-2 bg-slate-900/50 cursor-none"
                    >
                ...
                <div className="relative z-10 flex flex-col h-full">
                  ...
                  <div className="flex items-center justify-between border-b border-white/5 pb-4 mb-6">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-sm text-slate-450">{activeProject.title.toLowerCase().replace(' ', '_')}.config</span>
                    </div>
                  ...
                  <div className="flex-1 overflow-auto p-5 border border-white/5 bg-[#020617] font-mono text-xs md:text-sm leading-relaxed text-[#94a3b8] custom-scrollbar">
