...
const SKILLS = [
  ...
  { icon: Wrench, title: 'Tools', desc: 'Git, GitHub, Postman, Vercel, Docker', coord: 'SYS.SRC: TLS' },
  { icon: BookOpen, title: 'Core Concepts', desc: 'Data Structures & Algorithms, OOP, DBMS', coord: 'SYS.SRC: DYN' },
  { icon: Trophy, title: 'Profiles', desc: 'Leetcode: 1551 Max Rating, 133+ solved · Skillrack: 1009 problems', coord: 'SYS.SRC: PRF' },
];
...
function Label({ children }: { children: string }) {
  return (
    <div className="flex items-center gap-2 mb-4">
      ...
      <p className="font-mono text-[0.55rem] tracking-[0.3em] uppercase text-sky-400/70">
        {children}
      </p>
    </div>
  );
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-16 gap-8 border-b border-white/5 pb-12">
          <div className="text-left max-w-xl">
            ...
            <p className="font-body text-slate-400 text-sm md:text-base leading-relaxed">
              I am a software engineer focused on building robust backends, high-fidelity client views, and optimization of complex algorithmic workflows.
            </p>
          </div>

          <div className="flex flex-col text-left font-mono text-[0.62rem] text-slate-500 tracking-wider space-y-1.5 border-l border-white/10 pl-6">
            <div>[ USER_ID: SURYA_P ]</div>
            <div>[ STATUS: ONLINE_ACTIVE ]</div>
            <div>[ LOC_REF: INDIA // IN_ZONE ]</div>
          </div>
        </div>
        ...
        <div className="editorial-grid grid-cols-1 md:grid-cols-3 mb-24">
          {[
            ...
            <div key={i} className="grid-cell hud-brackets hud-scanline flex flex-col justify-between text-left py-8 min-h-36 hover:bg-sky-950/5 transition-colors">
              <div className="flex justify-between items-center font-mono text-[0.5rem] text-slate-500">
                <span>{s.ref}</span>
                <span className="text-sky-400/50">[ STATUS: OK ]</span>
              </div>
              <div className="font-display text-5xl font-black text-white tracking-tight my-4">{s.value}</div>
              <div className="font-mono text-[0.55rem] tracking-[0.2em] uppercase text-slate-400">{s.label}</div>
            </div>
          ))}
        </div>
          ...
          <div className="editorial-grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {SKILLS.map((sk, i) => (
              <div
                ...
                className="grid-cell hud-brackets hud-scanline flex flex-col justify-between group cursor-none hover:bg-sky-950/5 min-h-[220px]"
              >
                {/* Top status headers */}
                <div className="flex justify-between items-center font-mono text-[0.5rem] text-slate-500 mb-6">
                  <span>{sk.coord}</span>
                  <span className="text-sky-400/50 group-hover:text-sky-400 transition-colors">
                    [ ACTIVE.MOD ]
                  </span>
                </div>
                ...
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    <sk.icon className="w-4 h-4 text-slate-500 group-hover:text-sky-400 transition-colors" />
                    <h3 className="font-display text-base font-bold text-white tracking-wide group-hover:text-sky-400 transition-colors">
                      {sk.title}
                    </h3>
                  </div>
                ...
                {/* Bottom coordinates */}
                <div className="flex justify-between items-center font-mono text-[0.55rem] text-slate-600 mt-8">
